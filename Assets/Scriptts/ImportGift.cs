...
public class ImportGift : MonoBehaviour
{
    ...
    private void OnApplicationFocus(bool focus)
    {
        ...
        void CheckIncomingIntent()
        {
            ...
            {
                Debug.LogError(
                    "ImportGift: ошибка получения Intent:\n" +
                    e
                );
            }
        ...
        try
        {
            ...
            {
                throw new Exception(
                    "Не удалось получить Android Context"
                );
            }

            AndroidJavaClass importer =
                new AndroidJavaClass(
                    "com.clicklove.gamegift.GameGiftImporter"
                );

            Debug.Log(
                "ImportGift: копирование .gamegift..."
            );

            bool copied =
                importer.CallStatic<bool>(
                    "copyUriToFile",
                    context,
                    uriString,
                    giftFile
            ...
            {
                throw new Exception(
                    "Android не смог скопировать gamegift"
                );
            }
            ...
            {
                throw new Exception(
                    "received.gamegift не создан"
                );
            }

            Debug.Log(
                "ImportGift: файл успешно скопирован: " +
                giftFile
            );

            // Дальше уже обычная Unity-работа
            Directory.CreateDirectory(
                extractedFolder
            );

            Debug.Log(
                "ImportGift: распаковка..."
            );
            ...
            ExtractZipSafely(giftFile, extractedFolder);
                ...
                TestCreateCardController.TestSaveCards saveData = JsonUtility.FromJson<TestCreateCardController.TestSaveCards>(json);
                ...
                if (controller == null)
                {
                    controller = FindFirstObjectByType<TestCreateCardController>();
                }
                if (controller != null)
                {
                    controller.ImportCards(saveData.photos);
